...
def create_flow_graph(G):
    ...
    return F
# below is the min cut for vertex set with with a fixed vertex
...
# below is the min cut for vertex set by comparing the case of a fixed vertex above
def subproblem(G, x):
...
def modulus(G):
    ...
    H = G
    # print table header
    print("| {:^20} | {:^20} | {:^20} |".format("sigma^{-1}eta", "#node_shrunk", "#node_remain"))
    print( ("+" + "-"*22)*3 + "+")
    ...
    while len(H.nodes()) > 1:
        ...
        print("| {:^20} | {:^20} | {:^20} | ".format(np.round(theta,3), len(subgraph), remain))
...
# Reinforce problem
# below is the attack problem for e \in A
...
# below is the attack problem using the case e \in A: min f(A)-y(A): e \in A, x \in Pf
def subproblem_2(G, x, e):
    """
    Uses max flow to complete a step of Cunningham's algorithm on
    edge e.
...
#attack problem: return min x(A)+f(A-bar)
def attack(G, x):
    m =len(G.edges())
    y = np.zeros(m)
    for i,(u,v) in enumerate(G.edges()):
    ...
    inverse_enum =  [(u,v) for i,(u,v) in enumerate(G.edges())]
    for j in range(m):
        eps = subproblem_2(G, y, inverse_enum[j])
        if eps > x[j]-y[j]:
            eps = x[j]-y[j]
        y[j]=y[j]+eps
    return sum(y)
...
#reverse_attack: min x(A)-g(A), e \in A = min x(A)+f(A-bar)-f(E) , e \in A

def reverse_attack(G, x, i):
    # Create a copy of G to avoid modifying the original graph
    G_prime = G.copy()
    inverse_enum =  [(u,v) for i,(u,v) in enumerate(G_prime.edges())]
    # Find the edge with index i
    edge_to_remove = inverse_enum[i]
    
    # Remove the edge from G'
    G_prime.remove_edge(*edge_to_remove)
    inverse_enum =  [(u,v) for i,(u,v) in enumerate(G.edges())]
    # Create x' by removing the weight of the deleted edge
    #m = len(G.edges())  # Original number of edges
    x_prime = np.delete(x, i)  # Remove the i-th entry from x
    
    # Compute attack on the modified graph
    return x[i]- (len(G.nodes())-1) + attack(G_prime, x_prime)



# sparsification problem
# c here is the order index of the costs, not the costs
def sparsification(G,c):
    S = strength(G)
    for i,(u,v) in enumerate(G.edges()):
        G[u][v]['enum'] = i
    inverse_enum =  [(u,v) for i,(u,v) in enumerate(G.edges())]
    new_weight = [G[u][v]['weight'] for i,(u,v) in enumerate(G.edges())]
    for j in c:
        x =[new_weight[i]/S for i in range(len(new_weight))]
        eps = reverse_attack(G, x, j)*S
        new_weight[j] = new_weight[j] - eps
    return  new_weight
